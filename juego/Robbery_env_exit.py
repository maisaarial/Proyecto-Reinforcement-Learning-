...
class ThiefEnv_exit(gym.Env):
    ...
    def step(self, action):
        ...
        if self.has_object == 0:
            ...
            self.prev_dist = min(self.distance(self.exits[0]),self.distance(self.exits[1]),self.distance(self.exits[2]))
        ...
        if get_contact_object(self.sensor_thief, self.object_body):
            ...
            self.exit_ids = set_exit_tiles(height=0.5)
            self.prev_dist = min(self.distance(self.exits[0]),
                                self.distance(self.exits[1]),
                                self.distance(self.exits[2]))
            for e in self.exit_ids :
                self.types[e] = 3
                p.setCollisionFilterPair(e, self.sensor_thief, -1, -1, enableCollision=1)
            reward += 100 * (1 - self.current_steps / self.max_steps)
        ...
        if self.has_object == 0:
            goal_dist = self.distance(self.goal)
        else:
            goal_dist = min(self.distance(self.exits[0]),self.distance(self.exits[1]),self.distance(self.exits[2]))
