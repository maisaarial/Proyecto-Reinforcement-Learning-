...
class Multiagent_patrol(gym.Env):
    metadata = {"render_modes": ["human"]}

    def __init__(self, thief_model, max_steps=1000, render_mode=None):
        super().__init__()
    ...
    def calculate_reward(self):
        ...
        if get_contact_object(self.sensor_thief, self.object_body) and self.has_object == 0:
            ...
            for e in self.exit_ids :
                ...
                p.setCollisionFilterPair(e, self.sensor_patrol, -1, -1, enableCollision=1)
            reward -= 5
        return reward
    ...
    def step(self, action):
        ...
        if self.render_mode =="human" : 
            debug = True
        self.ray_results_patrol = raycast_view_cone(self.sensor_patrol, fov=np.pi/2, num_rays=11, max_distance=5.0, height=0.05, debug=debug)
        ...
        obs = {"ray_view":self._get_observation(self.ray_results_patrol),
               ...
               "patrol_pos" : self.new_patrol_pos / 14,
               "goal": self.goal / 14,
               "exits" : self.exits / 14,
               "has_object": self.has_object,
               "alert_flag": self.alert_flag}
    ...
    def _get_observation(self, ray_result):
        l= []
        for r in ray_result : 
            id = r[0]
    ...
    def update_camera_heatmap(self):
        for r in self.ray_results_patrol:
            hit_id = r[0]
    ...
    def observation_thief(self):
        ...
        obs = {"goal_vector":self.calculate_goal_vector(),
               ...
               "has_object": self.has_object,
               "alert_flag": self.alert_flag_thief}
        return obs
